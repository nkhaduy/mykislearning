...
async function apiFetch(path, options = {}, timeoutMs = 30000) {
  ...
  try {
    ...
    if (err.name === "AbortError") throw new Error(`API_TIMEOUT after ${timeoutMs}ms`, { cause: err });
    throw err;
  } finally {
    clearTimeout(timer);
  }
...
export const trainingApiService = {
  ...
  async syncParticipants(sessionId, participants, accountId, { mode = "merge" } = {}) {
    return apiFetch("/participants", {
      ...
      body: JSON.stringify({
        ...
        participants: participants.map((participant) => ({
          id: participant.id,
          session_id: sessionId,
          account_id: participant.accountId || participant.account_id,
          role: participant.role || "learner",
          status: participant.status || "assigned",
          created_at: participant.createdAt || participant.addedAt || new Date().toISOString(),
          source: participant.source || "manual",
          added_by: participant.addedBy || accountId,
        })),
      }),
    });
  },
