import { importedEmployees, importSummary } from "./runtimeFixtures.js";
...
const actor = { id: "system", name: "System", role: "system" };
export const DEMO_HR_EMAIL = "";
export const DEMO_HR_PASSWORD = "";
export const DEMO_EMPLOYEE_EMAIL = "";
export const DEMO_EMPLOYEE_PASSWORD = "";
export const HR_SUPPORT_NAME = "Learning Support";
export const HR_SUPPORT_EMAIL = "";

const demoHrAccount = null;
const seedAccounts = [];
const seedAudit = [];
...
export function getEmployees() {
  ...
  return merged.map((employee) => {
    const dynamicIssue = !employee.email
      ? "invalid_email"
      : emailCounts[employee.email.toLowerCase()] > 1
        ? "duplicate_email"
        : "";
...
export function createAccount(data) {
  const account = {
    ...
    accountStatus: "pendingActivation",
    passwordHash: data.temporaryPassword ? hashPassword(data.temporaryPassword) : "",
    passwordResetRequired: true,
    failedLoginAttempts: 0,
    lastLoginAt: null,
    lastFailedLoginAt: null,
...
/**
 * Sync a Supabase profile into the localStorage accounts store so that
 * getAccountById(supabaseUuid) works after a Supabase-based login.
 * Does NOT store a password hash — authentication is handled by Supabase Auth.
 */
export function syncSupabaseProfile(profile) {
  const accounts = readJson(ACCOUNT_KEY, seedAccounts);
  const idx = accounts.findIndex(
    (a) => a.id === profile.id || a.email?.toLowerCase() === profile.email?.toLowerCase()
  );
  const passwordStatus = profile.passwordStatus || profile.password_status || "normal";
  const entry = {
    id: profile.id,
    ...
    employeeCode: profile.employeeCode || profile.employee_code || "",
    department: profile.department || "",
    position: profile.position || "",
    passwordResetRequired: passwordStatus === "resetRequired",
    failedLoginAttempts: 0,
    lockedUntil: null,
...
/**
 * Fetch real data from Supabase and populate localStorage cache.
 * Call this once after login so all devices see the same data.
 * accountId and role are from the current session.
 */
export async function hydrateFromSupabase(accountId, role) {
  ...
  // Courses
  const coursesData = await safe(async () => {
    const res = await fetch("/api/courses", { headers: h });
    if (!res.ok) return null;
    return res.json();
  });
  if (Array.isArray(coursesData) && coursesData.length) {
    const existing = readArray(COURSES_KEY);
    const merged = [...existing];
    for (const c of coursesData) {
      const idx = merged.findIndex((x) => x.id === c.id);
      const mapped = { ...c, deliveryMode: c.deliveryMode || c.delivery_mode || "online" };
      if (idx >= 0) merged[idx] = { ...merged[idx], ...mapped };
      else merged.push(mapped);
    }
    writeJson(COURSES_KEY, merged);
  }

  // Enrollments
  const enrData = await safe(async () => {
    const url = isHr ? "/api/enrollments" : `/api/enrollments?accountId=${encodeURIComponent(accountId)}`;
    const res = await fetch(url, { headers: h });
    if (!res.ok) return null;
    return res.json();
  });
  if (Array.isArray(enrData) && enrData.length) {
    const existing = readArray(ENROLLMENTS_KEY);
    const merged = [...existing];
    for (const e of enrData) {
      const mapped = { ...e.data, id: e.id, courseId: e.courseId || e.course_id, accountId: e.accountId || e.account_id, status: e.status };
      const idx = merged.findIndex((x) => x.id === e.id || (x.courseId === mapped.courseId && x.accountId === mapped.accountId));
      if (idx >= 0) merged[idx] = { ...merged[idx], ...mapped };
      else merged.push(mapped);
    }
    writeJson(ENROLLMENTS_KEY, merged);
  }

  // Content progress (own account only)
  const progressData = await safe(async () => {
    const res = await fetch(`/api/content-progress?accountId=${encodeURIComponent(accountId)}`, { headers: h });
    if (!res.ok) return null;
    return res.json();
  });
  if (Array.isArray(progressData) && progressData.length) {
    const existing = readArray(CONTENT_PROGRESS_KEY);
    const merged = [...existing];
    for (const p of progressData) {
      const mapped = { ...p.data, id: p.id, contentId: p.contentId || p.content_id, courseId: p.courseId || p.course_id, accountId: p.accountId || p.account_id };
      const idx = merged.findIndex((x) => x.accountId === mapped.accountId && x.contentId === mapped.contentId);
      if (idx >= 0) merged[idx] = { ...merged[idx], ...mapped };
      else merged.push(mapped);
    }
    writeJson(CONTENT_PROGRESS_KEY, merged);
  }

  // Notifications
  const notifData = await safe(async () => {
    const res = await fetch(`/api/notifications?accountId=${encodeURIComponent(accountId)}`, { headers: h });
    if (!res.ok) return null;
    return res.json();
  });
  if (Array.isArray(notifData) && notifData.length) {
    const existing = readArray(NOTIFICATIONS_KEY);
    const merged = [...existing];
...
const seedCourses = [
  { id: "course-001", title: "Leadership Training Course", titleKr: "리더십 교육 과정", description: "Phát triển năng lực lãnh đạo, quản lý đội ngũ", category: "Kỹ năng mềm", format: "Offline", durationHours: 16, status: "published", createdBy: "system", createdAt: "2026-01-10 08:00", updatedAt: null, updatedBy: null, imageUrl: "/images/leadership-training-course.png" },
  { id: "course-002", title: "Communication Training Course", titleKr: "커뮤니케이션 교육 과정", description: "Kỹ năng giao tiếp, lắng nghe, phản hồi", category: "Kỹ năng mềm", format: "Offline", durationHours: 12, status: "published", createdBy: "system", createdAt: "2026-01-10 08:00", updatedAt: null, updatedBy: null, imageUrl: "/images/communication-training-course.png" },
  { id: "course-003", title: "Kiến thức chứng khoán cơ bản", titleKr: "", description: "Củng cố kiến thức nền tảng thị trường chứng khoán", category: "Chuyên môn", format: "Online", durationHours: 8, status: "published", createdBy: "system", createdAt: "2026-02-01 08:00", updatedAt: null, updatedBy: null },
  { id: "course-004", title: "Ôn tập Chứng chỉ hành nghề Môi giới chứng khoán", titleKr: "", description: "Lộ trình ôn tập và bộ đề luyện thi UBCKNN", category: "Chứng chỉ", format: "Hybrid", durationHours: 40, status: "published", createdBy: "system", createdAt: "2026-02-15 08:00", updatedAt: null, updatedBy: null },
  { id: "course-005", title: "Kỹ năng báo cáo và trình bày vấn đề", titleKr: "", description: "Chuẩn hóa kỹ năng báo cáo, phối hợp với cấp quản lý", category: "Kỹ năng mềm", format: "Online", durationHours: 6, status: "draft", createdBy: "system", createdAt: "2026-03-01 08:00", updatedAt: null, updatedBy: null },
];
...
export function createCourse(data) {
  ...
  // Supabase sync is handled by the caller (app.js) with the correct session.accountId
  return course;
}
...
export function createNotification(data) {
  ...
  fetch("/api/notifications", {
    ...
    body: JSON.stringify({ id: notification.id, account_id: notification.targetAccountId, type: notification.type, title: notification.title, body: notification.body, link: notification.actionUrl, is_read: false, created_by: notification.createdBy, expires_at: notification.expiresAt }),
  }).catch(() => {});
  return notification;
}
...
export function submitQuizAttempt({ attemptId, accountId, answers }) {
  ...
  writeJson(QUIZ_ATTEMPTS_KEY, attempts); createNotification({ type: "quiz_result", targetAccountId: accountId, title: pendingManual?"Bài kiểm tra đang chờ chấm":attempts[index].passed?"Bạn đã đạt bài kiểm tra":"Kết quả bài kiểm tra", body: `${quiz.title}: ${pendingManual?"Chờ chấm":`${scorePercent}%`}` });
  // Sync to Supabase
  const _attempt = attempts[index];
  fetch("/api/quizzes/attempts", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ quizId: _attempt.quizId, attempt: _attempt }),
  }).catch(() => {});
...
export function updateCourseContent(id, data) {
  ...
  rows[index]=updated;writeJson(COURSE_CONTENT_KEY,rows);apiSyncContent(updated.courseId,[updated],"hr");return updated;
}
...
export function saveContentProgress(data) {
  ...
  if(index>=0)rows[index]=next;else rows.push(next); writeJson(CONTENT_PROGRESS_KEY,rows); bumpTrainingRevision(); syncEnrollmentProgress(data.accountId,data.courseId); apiSyncProgress(next,data.accountId); return next;
}
