...
export function getEmployees() {
  ...
  const merged = importedEmployees.map((employee) => ({
    ...employee,
...
export function createCourseContent(data) {
  ...
  rows.push(item);writeJson(COURSE_CONTENT_KEY,rows);return item;
}
...
export function saveContentProgress(data) {
  ...
  if(index>=0)rows[index]=next;else rows.push(next); writeJson(CONTENT_PROGRESS_KEY,rows); syncEnrollmentProgress(data.accountId,data.courseId); return next;
}
