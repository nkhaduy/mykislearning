...
__name(requireHrCaller, "requireHrCaller");
...
var KNOWN_HR_IDS = /* @__PURE__ */ new Set(["acc-hr-demo", "acc-hr-001"]);
...
function requireHr(request) {
  const acct = resolveAccount(request);
  if (!acct || acct.role !== "hr") return null;
  return acct;
}
__name(requireHr, "requireHr");

// worker/routes/courses.js
var STORAGE_BUCKET = "course-content";
var SIGNED_URL_EXPIRES = 3600;
async function attachSignedUrls(supabase, items) {
  return Promise.all(
    items.map(async (item) => {
      if (item.sourceType !== "uploaded" || !item.storagePath) return item;
      const { data, error } = await supabase.storage.from(item.storageBucket || STORAGE_BUCKET).createSignedUrl(item.storagePath, SIGNED_URL_EXPIRES);
      if (error || !data?.signedUrl) return { ...item, sourceUrl: null, signedUrlError: error?.message };
      return { ...item, sourceUrl: data.signedUrl };
    })
  );
}
__name(attachSignedUrls, "attachSignedUrls");
async function handleCourses(request, env) {
  ...
  if (path === "/api/courses/content") {
    if (method === "GET") {
      ...
      const { data, error } = await supabase.from("course_content").select("id, course_id, type, sort_order, data").eq("course_id", courseId).order("sort_order", { ascending: true });
    ...
    if (method === "POST") {
      ...
      const rows = items.map((item, idx) => ({
        ...
        data: (() => {
          ...
          if (d.sourceType === "uploaded") delete d.sourceUrl;
          return d;
        })(),
  ...
  if (method === "GET") {
    ...
    if (!acct) return json({ error: "Unauthorized" }, 401);
    if (acct.role === "hr") {
      const { data, error } = await supabase.from("courses").select("id, status, delivery_mode, data, updated_at").order("updated_at", { ascending: false });
      if (error) return json({ error: error.message }, 500);
      return json((data || []).map((row) => ({ ...row.data, id: row.id, status: row.status, deliveryMode: row.delivery_mode })));
    } else {
      const { data: enrs, error: enrErr } = await supabase.from("enrollments").select("course_id").eq("account_id", acct.accountId);
      if (enrErr) return json({ error: enrErr.message }, 500);
      const ids = (enrs || []).map((e) => e.course_id);
      if (!ids.length) return json([]);
      const { data, error } = await supabase.from("courses").select("id, status, delivery_mode, data").in("id", ids).eq("status", "published");
      if (error) return json({ error: error.message }, 500);
      return json((data || []).map((row) => ({ ...row.data, id: row.id, status: row.status, deliveryMode: row.delivery_mode })));
    }
  }
...
// worker/routes/enrollments.js
async function handleEnrollments(request, env) {
  const method = request.method.toUpperCase();
  if (method === "OPTIONS") return corsPreflight();
  const url = new URL(request.url);
  const supabase = getSupabase(env);
  ...
  if (method === "PATCH") {
    ...
    const { error } = await supabase.from("enrollments").update({
      data: merged,
      status: merged.status || row.data?.status,
      updated_at: (/* @__PURE__ */ new Date()).toISOString()
    }).eq("id", row.id);
    if (error) return json({ error: error.message }, 500);
...
__name(handleEnrollments, "handleEnrollments");

// worker/routes/content-progress.js
async function handleContentProgress(request, env) {
  const method = request.method.toUpperCase();
  if (method === "OPTIONS") return corsPreflight();
  const url = new URL(request.url);
  const supabase = getSupabase(env);
  ...
  if (method === "GET") {
    ...
    let query = supabase.from("content_progress").select("id, content_id, account_id, course_id, data").eq("account_id", targetAccount);
    if (courseId) query = query.eq("course_id", courseId);
  ...
  if (method === "POST") {
    ...
    if (typeof progress.enrollmentProgressPercent === "number") {
      await supabase.from("enrollments").update({
        status: progress.enrollmentProgressPercent === 100 ? "completed" : progress.enrollmentProgressPercent > 0 ? "inProgress" : "notStarted",
        updated_at: (/* @__PURE__ */ new Date()).toISOString()
...
__name(handleContentProgress, "handleContentProgress");

// worker/routes/training.js
async function handleTraining(request, env) {
  ...
  if (path === "/api/training/sessions") {
    if (method === "GET") {
      ...
      if (acct.role !== "hr") {
        const { data: parts } = await supabase.from("training_participants").select("session_id").eq("account_id", acct.accountId);
  ...
  if (path === "/api/training/participants") {
    if (method === "GET") {
      ...
      const { data, error } = await supabase.from("training_participants").select("*").eq("session_id", sessionId).order("id", { ascending: true });
      if (error) return json({ error: error.message }, 500);
    ...
    if (method === "POST") {
      ...
      const normalizedByAccount = new Map(participants.map((p) => {
        ...
        return [accountId, {
          ...
          accountId,
          role: String(p.role || "learner"),
          status: String(p.status || "assigned"),
          createdAt: String(p.created_at || p.createdAt || (/* @__PURE__ */ new Date()).toISOString()),
      ...
      const { data: saved, error: verifyError } = await supabase.from("training_participants").select("id, session_id, account_id, data").eq("session_id", sessionId);
    ...
    if (method === "DELETE") {
      ...
      const { error } = await supabase.from("training_participants").delete().eq("session_id", sessionId).eq("account_id", accountId);
      if (error) return json({ error: error.message }, 500);
      return json({ ok: true });
    }
    return methodNotAllowed();
  ...
  if (path === "/api/training/calendar") {
    ...
    const { data: parts, error: partErr } = await supabase.from("training_participants").select("session_id").eq("account_id", accountId);
    ...
    const { data: sessions, error: sessErr } = await supabase.from("training_sessions").select("id, status, data").in("id", sessionIds).not("status", "in", '("draft")');
    if (sessErr) return json({ error: sessErr.message }, 500);
    const { data: regs, error: regErr } = await supabase.from("training_registrations").select("session_id, data").in("session_id", sessionIds).eq("account_id", accountId);
  ...
  if (path === "/api/training/registrations") {
    ...
    if (method === "GET") {
      const sessionId = url.searchParams.get("sessionId");
      if (!sessionId) return json({ error: "sessionId required" }, 400);
      const { data, error } = await supabase.from("training_registrations").select("*").eq("session_id", sessionId);
      if (error) return json({ error: error.message }, 500);
      return json((data || []).map((row) => ({ ...row.data, id: row.id })));
    }
    if (method === "POST") {
    ...
    if (method === "PATCH") {
      ...
      const { data: existing } = await supabase.from("training_registrations").select("data").eq("session_id", sessionId).eq("account_id", accountId).single();
...
async function handleAttendance(request, env) {
  ...
  if (path === "/api/attendance/check-in") {
    ...
    const { data: qrToken, error: tokenErr } = await supabase.from("qr_tokens").select("*, session_slots(*, training_sessions(*))").eq("token_hash", tokenHash).eq("action", action).eq("status", "open").single();
    ...
    const { data: participant } = await supabase.from("session_participants").select("id").eq("session_id", slot.session_id).eq("account_id", user.id).single();
...
__name(handleAttendance, "handleAttendance");

// worker/routes/external-training.js
async function handleExternalTraining(request, env) {
  const method = request.method.toUpperCase();
  if (method === "OPTIONS") return corsPreflight();
  const supabase = getSupabase(env);
  if (method === "GET") {
  ...
  if (method === "PATCH") {
    ...
    const { data, error } = await supabase.from("external_training_requests").update({
      status,
      hr_feedback: String(hr_feedback || ""),
      reviewed_by: acct.accountId,
...
__name(handleExternalTraining, "handleExternalTraining");

// worker/routes/employees.js
async function handleEmployees(request, env) {
  ...
  if (employeeId && !subResource && method === "PATCH") {
    ...
    if (!acct) return json({ error: "HR only" }, 403);
    const body = await readJson(request);
    const { error } = await supabase.from("profiles").update({ ...body, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", employeeId);
    if (error) return json({ error: error.message }, 500);
    return json({ ok: true });
  }
  if (employeeId && subResource === "certifications" && method === "GET") {
    ...
    const { data, error } = await supabase.from("employee_certifications").select("*").eq("account_id", employeeId).order("awarded_at", { ascending: false });
    if (error) return json({ error: error.message }, 500);
  ...
  if (employeeId && subResource === "certifications" && subId && method === "PATCH") {
    ...
    if (!acct) return json({ error: "HR only" }, 403);
    const body = await readJson(request);
    const { data, error } = await supabase.from("employee_certifications").update({ ...body, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", subId).eq("account_id", employeeId).select().single();
    if (error) return json({ error: error.message }, 500);
...
async function handleApiRequest(request, env) {
  ...
  try {
    ...
    if (path === "/api/auth" || path.startsWith("/api/auth/")) return handleAuth(request, env);
    if (path === "/api/courses" || path === "/api/courses/content") return handleCourses(request, env);
    if (path === "/api/enrollments") return handleEnrollments(request, env);
    if (path === "/api/content-progress") return handleContentProgress(request, env);
    if (path === "/api/training/sessions" || path === "/api/training/participants" || path === "/api/training/calendar" || path === "/api/training/registrations") return handleTraining(request, env);
    if (path.startsWith("/api/attendance/")) return handleAttendance(request, env);
    if (path === "/api/external-training" || path.startsWith("/api/external-training/")) return handleExternalTraining(request, env);
    if (path === "/api/employees" || path.startsWith("/api/employees/")) return handleEmployees(request, env);
