...
function render() {
  ...
  app.innerHTML = `<div class="page landing-page landing-entrance">
    <header class="header"><div class="container header-inner"><a class="brand" href="/"><img class="brand-logo" src="/legacy-public/assets/kis-logo-horizontal.png" alt="KIS" width="700" height="92" decoding="async"><span>MyKIS Learning</span></a><nav class="nav" id="public-nav" aria-label="${t.quickLinks}"><a href="/" aria-current="page" class="is-active">${t.home}</a><a href="/about-kis">${t.about}</a></nav><div class="header-actions">${languageSwitcher(language)}<button class="mobile-nav-toggle" type="button" data-mobile-nav-toggle aria-controls="public-nav" aria-expanded="false" aria-label="${t.menu}"><span aria-hidden="true">☰</span></button><a class="btn btn-primary header-mobile-cta" href="/login" style="font-size:13px;padding:0 14px;min-height:38px">${t.login}</a><a class="btn btn-primary btn--hero header-desktop-login" href="/login">${t.login}</a></div></div></header>
    <section class="hero hero--kis"><div class="hero-banner-wrap"><picture><source media="(max-width:767px)" srcset="/legacy-public/images/mykis-learning-banner-mobile.webp" type="image/webp" width="941" height="1672"><source media="(max-width:767px)" srcset="/legacy-public/images/mykis-learning-banner-mobile.png" width="941" height="1672"><source srcset="/legacy-public/images/mykis-learning-banner-desktop.webp" type="image/webp" width="1672" height="941"><img class="hero-banner-img" src="/legacy-public/images/mykis-learning-banner-desktop.png" alt="MyKIS Learning" width="1672" height="941" loading="eager" fetchpriority="high" decoding="sync"></picture></div><div class="container hero-overlay-content"><div class="hero-text"><span class="eyebrow">${t.eyebrow}</span><h1 class="hero-title--kis">${t.title}</h1><p class="hero-subtitle--kis">${t.subtitle}</p><div class="hero-actions hero-actions--kis"><a class="btn btn-primary btn--hero" href="/login">${t.loginSystem}</a></div></div></div></section>
...
export function mountLegacyHome(container) {
  app = container;
  render();
  return () => {
    cleanupMotion();
