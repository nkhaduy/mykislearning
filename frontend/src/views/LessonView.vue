<template>
  <div class="lesson-page" v-if="lesson"><RouterLink :to="`/courses/${route.params.courseId}`" class="back">← Course outline</RouterLink><article><p class="eyebrow">LESSON</p><h1>{{ lesson.title }}</h1><div class="lesson-content" v-html="html"></div><Button variant="solid" theme="gray" @click="complete" :loading="saving" :disabled="done">{{ done ? 'Completed' : 'Mark complete' }}</Button></article></div>
</template>
<script setup>
...
async function complete(){if(done.value)return;saving.value=true;try{await setLessonComplete({lessonId:lesson.value.id,courseId:route.params.courseId,userId:session.session.user.id,completed:true});done.value=true}finally{saving.value=false}}
</script>
