...
async function handleEmployees(request, env) {
  ...
  if (employeeId && !subResource && method === "PATCH") {
    ...
    if (!acct) return json({ error: "HR only" }, 403);
    const body = await readJson(request);
    const { error } = await supabase.from("profiles").upsert({ ...body, id: employeeId, updated_at: (/* @__PURE__ */ new Date()).toISOString() }, { onConflict: "id" });
    if (error) return json({ error: error.message }, 500);
    return json({ ok: true });
  }
  if (employeeId && subResource === "certifications" && method === "GET") {
    ...
    const { data, error } = await supabase.from("employee_certifications").select("*").eq("account_id", employeeId).order("created_at", { ascending: false });
    if (error) return json({ error: error.message }, 500);
  ...
  if (employeeId && subResource === "certifications" && subId && method === "DELETE") {
    const acct = requireHr(request);
    if (!acct) return json({ error: "HR only" }, 403);
    const { data, error } = await supabase.from("employee_certifications").update({ status: "revoked", revoked_at: (/* @__PURE__ */ new Date()).toISOString(), revoked_by: acct.accountId, updated_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("id", subId).eq("account_id", employeeId).select().single();
    if (error) return json({ error: error.message }, 500);
    return json({ certification: data });
  }
  return json({ error: "NOT_FOUND" }, 404);
...
async function handleApiRequest(request, env) {
  ...
  try {
    ...
    if (path === "/api/training/sessions" || path === "/api/training/participants" || path === "/api/training/calendar" || path === "/api/training/registrations") return await handleTraining(request, env);
