/**
 * POST /api/auth?action=create-user   → create employee account (HR only)
 * POST /api/auth?action=reset-password → reset password (HR only)
...
export default async function handler(req, res) {
  ...
  if (action === "create-user") {
    ...
    const { email, password, fullName, employeeCode, role = "employee", departmentId, position } = req.body;
    if (!email || !password || !fullName) return res.status(400).json({ error: "email, password, fullName are required" });
    if (!["hr", "employee"].includes(role)) return res.status(400).json({ error: "INVALID_ROLE" });
    ...
    const { error: profileErr } = await supabaseAdmin.from("profiles").insert({
      ...
      employee_code: employeeCode || null, role,
    ...
    await supabaseAdmin.from("audit_logs").insert({
      ...
      target_id: authData.user.id, result: "success", details: { email, role },
    });
