...
async function submitLogin(event) {
  ...
  try {
    ...
    if (!response.ok) { state.error = response.status === 429 || body.error === "RATE_LIMITED" ? "rate" : body.error === "ACCOUNT_LOCKED" ? "locked" : body.error === "ACCOUNT_INACTIVE" ? "inactive" : response.status >= 500 ? "system" : "invalid"; state.submitting = false; render(); return; }
