...
const copy = {
  vi: { title: "Đổi mật khẩu", current: "Mật khẩu hiện tại", next: "Mật khẩu mới", confirm: "Xác nhận mật khẩu mới", submit: "Cập nhật mật khẩu", success: "Mật khẩu đã được cập nhật. Các phiên khác đã bị thu hồi.", mismatch: "Mật khẩu xác nhận không khớp.", error: "Không thể đổi mật khẩu.", policy: "Ít nhất 6 ký tự và không trùng mật khẩu hiện tại." },
  en: { title: "Change password", current: "Current password", next: "New password", confirm: "Confirm new password", submit: "Update password", success: "Password updated. Other sessions were revoked.", mismatch: "The confirmation does not match.", error: "Unable to change password.", policy: "Use at least 6 characters and do not reuse the current password." },
  kr: { title: "비밀번호 변경", current: "현재 비밀번호", next: "새 비밀번호", confirm: "새 비밀번호 확인", submit: "비밀번호 업데이트", success: "비밀번호가 변경되었습니다. 다른 세션은 취소되었습니다.", mismatch: "비밀번호 확인이 일치하지 않습니다.", error: "비밀번호를 변경할 수 없습니다.", policy: "6자 이상이며 현재 비밀번호와 달라야 합니다." },
};
...
export async function mount({ account }) {
  ...
  app.innerHTML = `<main class="auth-page"><section class="auth-panel"><div class="auth-copy"><h1>${escapeHtml(text.title)}</h1><p>${escapeHtml(account?.fullName || "")}</p></div></section><section class="auth-visual"><form class="card login-card" data-change-password-form><h2>${escapeHtml(text.title)}</h2><label><span>${escapeHtml(text.current)}</span><input name="current" type="password" autocomplete="current-password" required></label><label><span>${escapeHtml(text.next)}</span><input name="next" type="password" autocomplete="new-password" minlength="6" maxlength="256" required></label><label><span>${escapeHtml(text.confirm)}</span><input name="confirm" type="password" autocomplete="new-password" minlength="6" maxlength="256" required></label><p>${escapeHtml(text.policy)}</p><p class="field-error" data-password-status role="alert" aria-live="polite"></p><button class="btn btn-primary" type="submit">${escapeHtml(text.submit)}</button></form></section></main>`;
