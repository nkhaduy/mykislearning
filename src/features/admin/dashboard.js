...
function renderOverview(data, text, locale) {
  ...
  <div class="admin-grid">
    <section class="route-card route-panel"><header class="admin-section-head"><div><p>Live</p><h2>${escapeHtml(text.live)}</h2></div><span>${live.length}</span></header><div class="admin-list">${live.map((item) => `<a href="${escapeAttribute(item.pagePath || "/hr/training-tracking")}"><span class="admin-avatar">${escapeHtml((item.fullName || "KIS").slice(0, 2).toUpperCase())}</span><div><strong>${escapeHtml(item.fullName || item.email || item.accountId || "—")}</strong><p>${escapeHtml(item.activityLabel || item.title || item.activityType || "—")}</p></div><time>${escapeHtml(formatDateTime(item.lastSeenAt, locale))}</time></a>`).join("") || `<p class="admin-empty">${escapeHtml(text.noData)}</p>`}</div></section>
    <section class="route-card route-panel"><header class="admin-section-head"><div><p>Follow-up</p><h2>${escapeHtml(text.inactive)}</h2></div><span>${inactive.length}</span></header><div class="admin-list">${inactive.map((item) => `<a href="/hr/employees"><span class="admin-avatar admin-avatar--muted">${escapeHtml((item.fullName || "KIS").slice(0, 2).toUpperCase())}</span><div><strong>${escapeHtml(item.fullName || item.email || "—")}</strong><p>${escapeHtml([item.department, item.status].filter(Boolean).join(" · "))}</p></div><time>${escapeHtml(formatDateTime(item.lastSeenAt, locale))}</time></a>`).join("") || `<p class="admin-empty">${escapeHtml(text.noData)}</p>`}</div></section>
  </div>
  <div class="admin-grid admin-grid--bottom">
    <section class="route-card route-panel"><header class="admin-section-head"><div><p>Calendar</p><h2>${escapeHtml(text.upcoming)}</h2></div><span>${upcoming.length}</span></header><div class="admin-session-list">${upcoming.map((item) => `<a href="/hr/sessions"><time>${escapeHtml(formatDateTime(item.startAt, locale))}</time><div><strong>${escapeHtml(item.title || item.courseTitle || "—")}</strong><p>${escapeHtml(item.locationName || item.mode || "")}</p></div></a>`).join("") || `<p class="admin-empty">${escapeHtml(text.noData)}</p>`}</div></section>
    <section class="route-card route-panel admin-quick"><header class="admin-section-head"><div><p>Action</p><h2>${escapeHtml(text.quick)}</h2></div></header><div><a href="/hr/employees">${escapeHtml(text.employeesLink)}<span>→</span></a><a href="/hr/courses">${escapeHtml(text.coursesLink)}<span>→</span></a><a href="/hr/reports">${escapeHtml(text.reportsLink)}<span>→</span></a></div></section>
  </div>`;
}
