...
export async function mount({ account }) {
  ...
  if (!["hr", "admin"].includes(account.role)) { shell.setContent(`<section class="route-card route-error"><h2>${escapeHtml(copy.restricted)}</h2></section>`); return; }
