...
const state = { language: readLanguage(), email: "", returnTo: readReturnTo(), support: null, supportType: "", supportError: "", supportSubmitting: false, supportDone: false, passwordVisible: false, error: "", submitting: false };
...
function isSafeReturnTo(route) {
  ...
  return /^(\/dashboard|\/admin|\/attendance\/scan|\/change-password)(?:[/?#]|$)/.test(route);
}
...
function readReturnTo() {
  ...
  const candidate = query === null ? stored : query;
...
function destinationLabel(t) {
  if (state.returnTo.startsWith("/admin")) return t.destinationAdmin;
...
async function submitLogin(event) {
  ...
  try {
    const response = await fetch("/api/auth?action=login", { method: "POST", headers: { "Content-Type": "application/json" }, credentials: "same-origin", body: JSON.stringify({ identifier: email, password, rememberMe: data.get("rememberMe") === "on" }) });
    const contentType = response.headers.get("content-type") || "";
    if (!/\bjson\b/i.test(contentType)) throw new Error("non_json");
    const body = await response.json();
    if (!response.ok) { state.error = body.error === "ACCOUNT_LOCKED" ? "locked" : body.error === "ACCOUNT_INACTIVE" ? "inactive" : "invalid"; state.submitting = false; render(); return; }
...
async function finishLogin(profile) {
  ...
  const role = profile.role === "hr" || profile.role === "admin" ? profile.role : "employee";
  const fallback = role === "hr" || role === "admin" ? "/admin" : "/dashboard";
  const target = profile.passwordStatus === "resetRequired" ? "/change-password" : (isSafeReturnTo(state.returnTo) ? state.returnTo : fallback);
